...
class RegistrationModel
{
    ...
    {
        // clean the input
        $user_name = substr(md5(strip_tags(Request::post('user_email'))), -8);
		    $user_full_name = strip_tags(Request::post('user_first_name')) . ' ' . strip_tags(Request::post('user_last_name'));
        ...
        $validation_result = self::registrationInputValidation($user_name, $user_password_new, $user_password_repeat, $user_email, $user_email_repeat, $user_full_name);
        ...
        if (!self::writeNewUserToDatabase($user_name, $user_password_hash, $user_email, time(), $user_activation_hash, $user_full_name)) {
        ...
        if (self::sendVerificationEmail($user_id, $user_email, $user_activation_hash)) {
            if($_ENV['VERIFY_EMAIL'] == 1){
                Session::add('feedback_positive', Text::get('FEEDBACK_ACCOUNT_SUCCESSFULLY_CREATED'));
            }else{
                Session::add('feedback_positive', "Your account was created successfully - You can now log in!");
            }
            return true;
        }
     ...
     * @return bool
     */
    public static function registrationInputValidation($user_name, $user_password_new, $user_password_repeat, $user_email, $user_email_repeat, $user_full_name)
    {
        $return = true;
    ...
    public static function writeNewUserToDatabase($user_name, $user_password_hash, $user_email, $user_creation_timestamp, $user_activation_hash, $user_full_name)
    {

        $database = DatabaseFactory::getFactory()->getConnection();
        if($_ENV['VERIFY_EMAIL'] == "false"){
    ...
    public static function sendVerificationEmail($user_id, $user_email, $user_activation_hash)
    {
        if($_ENV['VERIFY_EMAIL'] == "false"){
            ...
            $template = file_get_contents("email_template/email.html");
            $variables['subject'] = Config::get('EMAIL_VERIFICATION_SUBJECT');
            $variables['body'] = Config::get('EMAIL_VERIFICATION_CONTENT');
            $variables['action'] = "Verify Account";
            $variables['disclaimer'] = 'If you did not request an account, please dismiss this email';
            $variables['siteName'] = Request::siteDetails("siteTitle");
            $variables['link'] = Config::get("URL") . Config::get('EMAIL_VERIFICATION_URL') . '/' . urlencode($user_id) . '/' . urlencode($user_activation_hash);
            $variables['url'] = Config::get("URL");

            $subject = str_replace('{{ siteName }}', Request::siteDetails("siteTitle"), Config::get('EMAIL_VERIFICATION_SUBJECT'));
            ...
            $mail_sent = $mail->sendMail($user_email, Config::get('EMAIL_VERIFICATION_FROM_EMAIL'),
                Config::get('EMAIL_VERIFICATION_FROM_NAME'), $subject, $body
            );
