...
class PasswordResetModel
{
    ...
    public static function sendPasswordResetMail($user_name, $user_password_reset_hash, $user_email)
    {
        ...
        $variables['subject'] = Config::get('EMAIL_PASSWORD_RESET_SUBJECT');
        $variables['body'] = Config::get('EMAIL_PASSWORD_CONTENT');
        $variables['action'] = "Reset Password";
        $variables['disclaimer'] = 'If you did not request a password reset, please dismiss this email';
        $variables['siteName'] = Request::siteDetails("siteTitle");
        $variables['link'] = Config::get("URL") . Config::get('EMAIL_PASSWORD_RESET_URL') . '/' . urlencode($user_name) . '/' . urlencode($user_password_reset_hash);
        $variables['url'] = Config::get("URL");

        $subject = str_replace('{{ siteName }}', Request::siteDetails("siteTitle"), Config::get('EMAIL_PASSWORD_RESET_SUBJECT'));
        ...
        $mail_sent = $mail->sendMail($user_email, Config::get('EMAIL_PASSWORD_RESET_FROM_EMAIL'),
            Config::get('EMAIL_PASSWORD_RESET_FROM_NAME'), $subject, $body
        );
