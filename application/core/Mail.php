...
class Mail
{
     ...
     */
    public function sendMailWithMailgun($user_email, $from_email, $from_name, $subject, $body)
    {

        if(Config::get("EMAIL_MAILGUN_SERVER") == "eu"){
          $endpoint = "https://api.eu.mailgun.net/v3";
        }else{
          $endpoint = "https://api.mailgun.net/v3";
        }
        # Instantiate the client.
        $mgClient = Mailgun::create(Config::get("EMAIL_MAILGUN_API"), $endpoint);
        $domain = Config::get("EMAIL_MAILGUN_DOMAIN");
        $params = array(
        ...
        $wasSendingSuccessful = $mgClient->messages()->send($domain, $params);
    ...
    public function sendMail($user_email, $from_email, $from_name, $subject, $body)
    {
        ...
        if (Config::get('EMAIL_USED_MAILER') == "mailgun") {

            // returns true if successful, false if not
            return $this->sendMailWithMailgun(
                $user_email, $from_email, $from_name, $subject, $body
            );
        }
